...
class TestDlg(FormViewPartialLoadDlg, ui.Ui_testDlg):
    ...
    def loadPartialRecords(self):
        ...
        self.records = Test.objects.filter(start_date__range=(start_date, stop_date)).order_by("start_date", "renewal", "test_num")

    def getRecordDate(self, id):
        try:
            record = Test.objects.filter(renewal_id=id).order_by("-start_date").first()
            return record.start_date if record else None
        except:
            return None

    def getLatest(self, partial_id):
        max_id = partial_id.ljust(RENEWAL_ID_WIDTH, '9')
        min_id = partial_id.ljust(RENEWAL_ID_WIDTH, '0')
        record = Test.objects.filter(renewal__id__range=(int(min_id), int(max_id)), renewal__id__startswith=int(partial_id)).order_by("-start_date").first()
        return record if record else None
    
    def loadForm(self, record):
        try:
            dentist = Dentist.objects.get(id=RenewalToDentistID(record.renewal.id))
            self.setWindowTitle(f"SMS Test - {dentist.getFullName()}")
        except:
        ...
        match record.result:
            ...
            case _:
                self.testResultPushButton.setText(record.result)
        self.notifyPushButton.setEnabled(self.testResultPushButton.text() == "+")

        match record.control_result:
            case None:
                self.controlResultPushButton.setText("")
            case _:
                self.controlResultPushButton.setText(record.control_result)
        ...
        match record.strip_num:
            case None:
                self.stripNumLineEdit.setText("")
            case _:
                self.stripNumLineEdit.setText(str(record.strip_num))
    ...
    def startTest(self, id, strip, renewal, tests):
        try:
            ...
            try:
                ...
                ttl = "Database Error"
                msg = "Cannot find matching lot %d." % RenewalToLotID(id)
                assert False
            try:
